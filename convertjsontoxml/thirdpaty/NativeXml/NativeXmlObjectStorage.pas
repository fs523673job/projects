...
type
  ...
  // to an XML node.
  TsdXmlObjectWriter = class(TsdDebugPersistent)
  protected
  ...
  // from an XML node.
  TsdXmlObjectReader = class(TsdDebugPersistent)
  private
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        AComponentNode := AChildNode.NodeNew(C.Components[i].ClassName);
        if length(C.Components[i].Name) > 0 then
          AComponentNode.AttributeAdd('Name', C.Components[i].Name);
  ...
  if AObject is TCollection then
    ...
    begin
      AChildNode := ANode.NodeNew(TCollection(AObject).Items[i].ClassName);
...
var
  ...
  begin
    AChildNode := ANode.NodeNew(UnicodeString(PPropInfo(PropInfo)^.Name));
  end;

  //local
  procedure WriteInteger(Value: Int64);
  begin
    AChildNode.Value := IntToStr(Value);
  end;
  ...
  begin
    ...
    if not (Value = 0) then
      ANode.WriteFloat(UnicodeString(PPropInfo(PropInfo)^.Name), Value);
  end;
  ...
  begin
    ...
    if not (Value = 0) then
      ANode.WriteInt64(UnicodeString(PPropInfo(PropInfo)^.Name), Value);
  end;
  ...
  begin
    ...
    if not (length(Value) = 0) then
      ANode.WriteString(UnicodeString(PPropInfo(PropInfo)^.Name), Value);
  end;
  ...
  begin
    ...
    begin
      ...
      varOleStr:  AChildNode.Value := AValue;
      varString:  AChildNode.Value := AValue;
      varByte,
      varSmallInt,
...
var
  ...
  begin
    ...
    if (Result) then // Remove dumb hint
      repeat
        P := Pos(',', S);
        if P > 0 then
        begin
...
begin
  ...
  begin
    PropType := PPropInfo(PropInfo)^.PropType^;
    AChildNode := ANode.NodeByName(UnicodeString(PPropInfo(PropInfo)^.Name));
    if assigned(AChildNode) then
    begin
      ...
      {$else}
      tkWString:     SetWideStrProp(AObject, PropInfo, AChildNode.Value);
      {$endif}
      {$ifdef UNICODE}
