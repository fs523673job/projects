...
typedef struct _litePDFContext
{
   ...
   unsigned int sign_signatureHash;
                                                      ...
                                                      void *on_error_user_data)
{
   ...
   PdfStream::eDefaultFilter = ePdfFilter_None;
#endif
                                                       ...
                                                       unsigned int signatureHash)
{
   ...
   case LITEPDF_SIGNATURE_HASH_SHA1:
   case LITEPDF_SIGNATURE_HASH_SHA256:
   case LITEPDF_SIGNATURE_HASH_SHA384:
   case LITEPDF_SIGNATURE_HASH_SHA512:
      ctx->sign_signatureHash = signatureHash;
      break;
   default:
      if (ctx->on_error) {
         ctx->on_error(ERROR_INVALID_PARAMETER, "litePDF_SetSignatureHash: Unknown signature hash algorithm used", ctx->on_error_user_data);
      }
      return FALSE;
   }
      ...
      if (sigField) {
         ...
         if (len > 0) {
            if (len > signer.GetSignatureSize()) {
               ...
               oss << "Requires at least " << len << " bytes for the signature, but reserved is only " << signer.GetSignatureSize() << " bytes. Use litePDF_SetSignatureSize() to reserve more bytes.";
...
class MSignContext
{
 ...
 public:
   ...
   bool prepare(void)
   {
      ...
      const EVP_MD *hashAlgo = NULL;
      switch(ctx->sign_signatureHash) {
      case LITEPDF_SIGNATURE_HASH_SHA1:
         hashAlgo = EVP_sha1();
         break;
      case LITEPDF_SIGNATURE_HASH_SHA256:
      ...
      std::vector<MSignerData>::iterator it, end = ctx->sign_signers.end();
      for (it = ctx->sign_signers.begin(); it != end; it++) {
         if (!PKCS7_sign_add_signer(p7, it->cert, it->pkey, hashAlgo, 0/*!reserveBytes ? 0 : PKCS7_REUSE_DIGEST*/)) {
            std::string errs = getOpenSSLErrors();
