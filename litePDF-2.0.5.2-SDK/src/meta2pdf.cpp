...
#define double_equal2(a, b) (((a) - 1e-2) <= (b) && ((a) + 1e-2) >= (b))
...
static void LogicalUnitToPdf(struct processData *pd, int luX, int luY, double &pdfX, double &pdfY, bool translate = true, bool isText = false, bool ignoreMatrix = false)
{
   pdfX = 0.0;
   pdfY = 0.0;
   ...
   if (!ignoreMatrix && pd->currentDCState.worldMatrixValid && pd->currentDCState.worldMatrixApplied) {
      if (double_equal2 (pd->currentDCState.worldMatrix.eM11, 1.0) && double_equal2 (pd->currentDCState.worldMatrix.eM12, 0.0) &&
          double_equal2 (pd->currentDCState.worldMatrix.eM21, 0.0) && double_equal2 (pd->currentDCState.worldMatrix.eM22, 1.0)) {
...
static void maybePushWorldMatrix(struct processData *pd)
{
   ...
   bool isOnlyTranslate = double_equal2 (pd->currentDCState.worldMatrix.eM11, 1.0) && double_equal2 (pd->currentDCState.worldMatrix.eM12, 0.0) &&
                          double_equal2 (pd->currentDCState.worldMatrix.eM21, 0.0) && double_equal2 (pd->currentDCState.worldMatrix.eM22, 1.0);
      ...
      if (rgndata) {
         ...
         if (GetRegionData(pd->currentDCState.clipRgn, bytes, rgndata) &&
            ...
            for (ii = 0; ii < rgndata->rdh.nCount; ii++, rects++) {
               ...
               LogicalUnitToPdf(pd, r.left, r.top, dx, dy, true, false, true);
               LogicalUnitToPdf(pd, r.right, r.bottom, w, h, true, false, true);
...
static int CALLBACK processEnhMetaFileCB(HDC hDC, HANDLETABLE *lpHTable, const ENHMETARECORD *lpEMFR, int nObj, LPARAM lpData)
{
   ...
   case EMR_MODIFYWORLDTRANSFORM: {
      ...
      if (pd->currentDCState.worldMatrixValid &&
          double_equal2 (pd->currentDCState.worldMatrix.eM11, 1.0) && double_equal2 (pd->currentDCState.worldMatrix.eM12, 0.0) &&
          double_equal2 (pd->currentDCState.worldMatrix.eM21, 0.0) && double_equal2 (pd->currentDCState.worldMatrix.eM22, 1.0) &&
          double_equal2 (pd->currentDCState.worldMatrix.eDx,  0.0) && double_equal2 (pd->currentDCState.worldMatrix.eDy,  0.0)) {
         pd->currentDCState.worldMatrixValid = false;
      }
   } break;
   case EMR_MOVETOEX: {
   ...
   case EMR_SETWORLDTRANSFORM: {
      ...
      if (double_equal2 (dMatrix.eM11, 1.0) && double_equal2 (dMatrix.eM12, 0.0) &&
          double_equal2 (dMatrix.eM21, 0.0) && double_equal2 (dMatrix.eM22, 1.0) &&
          double_equal2 (dMatrix.eDx,  0.0) && double_equal2 (dMatrix.eDy,  0.0)) {
         pd->currentDCState.worldMatrixValid = false;
      } else {
