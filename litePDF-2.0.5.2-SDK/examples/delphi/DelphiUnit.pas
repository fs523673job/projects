...
uses
   ...
   Vcl.Dialogs, Vcl.StdCtrls, litePDF, Vcl.ExtCtrls, System.IOUtils, System.Math,
...
procedure TDelphiFrm.Button1Click(Sender: TObject);
var
  fileName: String;
...
begin
  ...
  try
    lpdf := TLitePDF.Create;
    lpdf.LoadFromFile(FileName, '', True, True);

    var resourceID := createResource(lpdf);

    var hDC := lpdf.AddPage(Trunc(lpdf.MMToUnit(210)),
                        Trunc(lpdf.MMToUnit(297)),
                        2100, 2970, LongWord(LitePDFDrawFlag_None));
    ...
    // save to file
    lpdf.SaveToFile(FileName + '_Desenho.pdf');
...
procedure TDelphiFrm.Button2Click(Sender: TObject);
var
  fileName: String;
begin
  if OpenDialog1.Execute then
  ...
  var dir  := TPath.GetDirectoryName(fileName);
  var base := TPath.GetFileNameWithoutExtension(fileName);
  var ext  := TPath.GetExtension(fileName);
  var outPutFile := TPath.Combine(dir, base + '_image' + Ext);
