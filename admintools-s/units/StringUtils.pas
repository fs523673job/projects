...
// Formatierung ohne Exception bei Fehler
...
uses System.StrUtils, System.Masks, System.Classes;
...
(* Umwandeln von ISO-8859-Zeichen für eine Sortierung mit Großbuchastaben*)
function ANSISortUpper (const S : string) : string;
begin
  Result:=ANSISortConvert(AnsiUpperCase(s));
  end;
...
begin
  ...
  else Result:=Format(sMany,[n]);
  end;

function GetPluralString (const sNo,sOne,sMany : string; n : integer; const s : string) : string;
begin
  if n=1 then Result:=Format(sOne,[s])
  else if (n=0) and (length(sNo)>0) then Result:=sNo
  else Result:=Format(sMany,[n,s]);
  end;
