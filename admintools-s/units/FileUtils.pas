...
{ ---------------------------------------------------------------- }
// Prüfe, ob Textdatei vorhanden
function ExistsFile (var f : TextFile) : boolean;

// Prüfen, ob in einem Verzeichnis eine Datei existiert, die der Maske entspricht
...
{ ---------------------------------------------------------------- }
// Bei nicht leeren Pfaden Backslash ergänzen
...
// Ändern einer Dateinamenerweiterung
function NewExt (Name,Ext  : string) : string;

// Hinzufügen einer Dateinamenerweiterung
function AddExt (const Name,Ext  : string; Always : boolean = false) : string;

// Hinzufügen eines Pfades zum Dateinamen
...
// Suffix zu Dateinamen hinzufügen
function AddNameSuffix (FName,Suffix,Ext : string) : string;

// Suffix zu Dateinamen mit Extension hinzufügen
function InsertNameSuffix (FName,Suffix : string) : string;

(* Suffix (Länge: CharCount) von Dateinamen mit Extension entfernen *)
...
// Extrahiere den übergeordneten Pfad
...
(* prüfe, ob alle Zeichen in APath zulässig sind *)
function CheckPathChars (APath : string) : boolean;

// prüfe, ob alle Zeichen in Filename zulässig sind
function CheckFilename (const Filename : string) : boolean;

// Ersetze ungültige Zeichen in Filename durch AChar
...
{ ---------------------------------------------------------------- }
// kein spezielles Verzeichnis (selbst und eins zurück)
...
{ ---------------------------------------------------------------- }
(* Diese Funktion prüft, ob eine Text-Datei vorhanden ist und öffnet sie *)
function ExistsFile (var f : TextFile) : boolean;
begin
  (*$i-*) reset (f) (*$i+*);
  Result:=ioresult=0;
...
{ --------------------------------------------------------------- }
(* Ändern einer Dateinamenerweiterung (sucht letzten Punkt - JR 9.8.05) *)
function NewExt (Name,Ext  : string) : string;
begin
  Name:=DelExt(Name);
...
{ ---------------------------------------------------------------- }
(* Suffix und Erweiterung zu Dateinamen hinzufügen *)
function AddNameSuffix (FName,Suffix,Ext : string) : string;
begin
...
(* prüfe, ob alle Zeichen in Filename (ohne Pfad) zulässig sind *)
function CheckFilename (const Filename : string) : boolean;
var
  i : integer;
begin
...
// Ersetze ungültige Zeichen in Filename (ohne Pfad) durch AChar
function ReplaceIllegalChars (const Filename : string; AChar : char) : string;
var
  i : integer;
begin
...
// DCount: Anzahl gelöschte Verzeichnisses
// FCount: Anzahl gelöschte Dateien
