...
function SizeToStr (Bytes : int64; DecSep : char = #0) : string;
...
var
  ...
  fs: TFormatSettings;
begin
  if Bytes<1024 then Result:=IntToStr(Bytes)+' '+rsBytes  // bytes
  else begin
    fs:=FormatSettings;
    if DecSep<>#0 then fs.DecimalSeparator:=DecSep;
    v:=Bytes/1024;
    if v<1024 then begin
      if v<100 then n:=2 else n:=1;
      Result:=Format('%.'+IntToStr(n)+'f ',[v],fs)+rsKBytes; // kBytes
      end
    else begin
      v:=v/1024;
      if v<1024 then begin
        if v<100 then n:=2 else n:=1;
        Result:=Format('%.'+IntToStr(n)+'f ',[v],fs)+rsMBytes  // MBytes
        end
      else Result:=Format('%.2f ',[v/1024],fs)+rsGBytes; // GBytes
      end;
    end;
  end;
