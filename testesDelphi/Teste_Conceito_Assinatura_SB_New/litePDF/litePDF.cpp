...
void TLitePDF::SetSignatureHash(TLitePDFSignatureHash signatureHash)
{
   const char *_func = "TLitePDF::SetSignatureHash";

   ensureLibraryLoaded(_func);

   ThrowIfFail(lib != NULL);

   InitFunc(BOOL, litePDF_SetSignatureHash, (void *pctx, unsigned int signatureHash));

   unsigned int hashAlgo;

   if (signatureHash == LitePDFSignatureHash_SHA1)
      hashAlgo = 1;
   else if (signatureHash == LitePDFSignatureHash_SHA256)
      hashAlgo = 2;
   else if (signatureHash == LitePDFSignatureHash_SHA384)
      hashAlgo = 3;
   else // LitePDFSignatureHash_SHA512
      hashAlgo = 4;


   ThrowLastErrorIfFail(func(context, hashAlgo));
}
