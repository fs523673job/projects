unit unHorizontalSignatureImageImp;

interface

uses
  unSignatureImage, unSignatureDTO, Vcl.Graphics, Vcl.Imaging.jpeg, unFunctions;

type
  THorizontalSignatureImage = class(TInterfacedObject, ISignatureImage)
  public
...
{ THorizontalSignatureAndStamp }

procedure THorizontalSignatureImage.AdjustImageSize(
  const AImage, ANewImage: TJPEGImage; const AFieldHeight,
  AFieldWidth: Integer; const AHeighPercent, AWidthPercent: Double);
var
...
function THorizontalSignatureImage.TreatSignatureImage(const ASignatureDTO: TSignatureDTO): TBitmap;
var
  NewSignature: TJPEGImage;
  NewFreeHeight, StampSpace: Integer;
begin
  Result := TBitmap.Create;
  NewSignature := TJPEGImage.Create;
  try
    StampSpace := IfThen((ASignatureDTO.StampModelID = 2), 40, 35);
    NewFreeHeight := IfThen((ASignatureDTO.FieldHeight - StampSpace > 0), (ASignatureDTO.FieldHeight - StampSpace), ASignatureDTO.FieldHeight);
    AdjustImageSize(ASignatureDTO.SignatureImage, NewSignature, NewFreeHeight, ASignatureDTO.FieldWidth, 1, 1);
    Result.SetSize(ASignatureDTO.FieldWidth, ASignatureDTO.FieldHeight);
    Result.Canvas.CopyRect(Rect(0, 0, NewSignature.Width, NewSignature.Height), NewSignature.Canvas, Rect(0, 0, NewSignature.Width, NewSignature.Height));
  finally
    NewSignature.Free;
  end;
end;
