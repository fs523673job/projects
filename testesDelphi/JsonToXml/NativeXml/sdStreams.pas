  ...
  begin
    ...
    begin
      if Result > Count then
        Result := Count;
      Move(Pointer(NativeUInt(FMemory) + NativeUInt(FPosition))^, Buffer, Result);
...
begin
  ...
  begin
    ...
    begin
      ...
      System.Move(Buffer, Pointer(NativeUInt(FMemory) + NativeUInt(FPosition))^, Count);
      FPosition := NewPos;
      Result := Count;
      Exit;
    end;
...
begin
  ...
  begin
    Move(Src^, FRawBuffer[FRawPosition], FChunkSize - FRawPosition);
    WriteChunk(FChunkSize);
    Dec(Siz, FChunkSize - FRawPosition);
    Inc(Src, FChunkSize - FRawPosition);
    FRawPosition := 0;
  end;

  // copy the raw buffer
  Move(Src^, FRawBuffer[FRawPosition], Siz);
