(* Delphi Komponente
   ...
   Vers. 1 - Sep. 1998
         1.1 - Okt. 2001
         1.2 - Jun. 2005  : verbesserte Bereichs¸berpr¸fung
...
type
  ...
  private
    ...
    FRangeError,
    FRangeCheck : boolean;
    procedure SetValue(i : int64); override;
    procedure SetMinValue(i : int64);
    function CutValue (NewValue : int64) : int64;
    function CheckValue (NewValue: int64): boolean;
    procedure SetMaxValue(i : int64);
    procedure WMKeyUp(var Message: TWMKeyDown); message WM_KEYUP;
...
//    function GetValue : extended;
    procedure SetValue(x : extended); override;
    procedure SetMinValue(x : extended);
    procedure SetMaxValue(x : extended);
    function CutValue (NewValue: extended): extended;
...
begin
  inherited Create(AOwner);
  FNumMode:=nmDecimal; FInpNumMd:=nmDecimal;
  FDigits:=0;
...
procedure TRangeEdit.SetMinValue(i : int64);
begin
  FMinValue:=i;
  FValue:=CutValue(FValue);
  end;

procedure TRangeEdit.SetMaxValue(i : int64);
begin
  FMaxValue:=i;
  FValue:=CutValue(FValue);
  end;

procedure TRangeEdit.SetValue(i : int64);
begin
  if FRangeCheck then i:=CutValue(i);
  inherited SetValue(i);
  end;

function TRangeEdit.CutValue (NewValue : int64) : int64;
begin
  Result:=NewValue;
...
procedure TFloatRangeEdit.SetMinValue(x : extended);
begin
  FMinValue:=x;
  FValue:=CutValue(FValue);
  end;

procedure TFloatRangeEdit.SetMaxValue(x : extended);
begin
  FMaxValue:=x;
  FValue:=CutValue(FValue);
  end;

procedure TFloatRangeEdit.SetValue(x : extended);
begin
  if FRangeCheck then x:=CutValue(x);
  inherited SetValue(x);
  end;
...
function TFloatRangeEdit.CutValue (NewValue: extended): extended;
begin
  Result:=NewValue;
  if not (csLoading in Componentstate) and not (csDesigning in Componentstate)
      and (FMaxValue<>FMinValue) then begin
...
begin
  x:=GetValueFromInput;
  SetValue(x);
  if FRangeCheck then CheckValue(x);
//    MessageBeep(MB_ICONERROR);
//    SetFocus;
...
procedure TCustomFloatComboBox.SetValue(x : extended);
begin
  FValue:=x;
  Text:=ValToStr(x);
  end;
