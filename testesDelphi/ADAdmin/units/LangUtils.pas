...
uses System.Classes, System.SysUtils, Vcl.Menus, WinApiUtils, IniFileUtils;
...
// Befehlszeile
  LangOption = 'lang';      // Spracheinstellung
  AltIni = 'ini';           // Ort für alternative Ini-Datei
...
uses
  Winapi.Windows, Vcl.Forms, Winapi.ShlObj, System.IniFiles, GnuGetText,
  UnitConsts, StringUtils, WinUtils, NumberUtils, FileUtils, WinShell, InitProg,
  Placeholders;
...
begin
  ...
  else Result:=pointer(BytesToCardinal(0,0,ord(LangCode[2]),ord(LangCode[1])));
  end;

function TLanguageList.CardinalToLangCodeString (Value : cardinal) : TLangCodeString;
begin
  if Value=0 then Result:=''
  else Result:=AnsiChar(TLongWord(Value).LoL)+AnsiChar(TLongWord(Value).LoH);
  end;
...
var
  pli : integer;
begin
  pli:=GetUserDefaultUILanguage and $3FF;       // not available with Win98
  if pli=0 then pli:=GetUserDefaultLangID and $3FF;
  case pli of
  $02 : Result:='bg';
  $05 : Result:='cs';
  $06 : Result:='da';
...
begin
  ...
  for j:=1 to ParamCount do if not ok then begin   // prüfe Befehlszeile
    ...
    if (s[1]='/') or (s[1]='-') then begin
      delete (s,1,1);
      if ReadOptionValue(s,LangOption) then Result:=s  // Sprache
      else if ReadOptionValue(s,AltIni) then si:=ReplacePathPlaceHolder(s);  // anderer Ort für Ini-Datei
      end;
    end;
  ...
  if length(CfgName)=0 then CfgName:=NewExt(ExtractFileName(Application.ExeName),CfgExt);
  CfgName:=AddPath(si,CfgName);
