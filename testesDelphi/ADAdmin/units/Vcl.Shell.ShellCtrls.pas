...
{                                                         	 }
{************************************************************}
{ Changes - J. Rathlev, info(a)rathlev-home.de 
...
//  Flags := SFGAO_DISPLAYATTRMASK;
  ...
  // following line changed, J. Rathlev (rathlev(a)physik.uni-kiel.de)
...
var
  ...
  begin
    if ColCount > 0 then begin
      if (Length(FColWidths)>0) and (FColWidths[0]>0) then n:=FColWidths[0] else n:=25;
      AddDefaultColumn(SShellDefaultNameStr, taLeftJustify, n);
      end;
    if ColCount > 1 then begin
      if (Length(FColWidths)>1) and (FColWidths[1]>0) then n:=FColWidths[1] else n:=10;
      AddDefaultColumn(SShellDefaultSizeStr, taRightJustify, n);
      end;
    if ColCount > 2 then begin
      if (Length(FColWidths)>2) and (FColWidths[2]>0) then n:=FColWidths[2] else n:=10;
      AddDefaultColumn(SShellDefaultTypeStr, taLeftJustify, n);
      end;
    if ColCount > 3 then begin
      if (Length(FColWidths)>3) and (FColWidths[3]>0) then n:=FColWidths[3] else n:=14;
