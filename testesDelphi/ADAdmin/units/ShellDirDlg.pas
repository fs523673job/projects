...
uses System.IniFiles, Vcl.Dialogs, System.StrUtils, Winapi.ShlObj, Winapi.Shellapi,
  Winapi.ActiveX, WinShell, WinUtils, FileUtils, IniFileUtils, GnuGetText, SelectDlg;
...
begin
  if (length(FIniName)>0) and (length(FIniSection)>0) then begin
    EraseSectionOfIniFile(FIniName,FIniSection);
...
begin
  ...
  n:=SelectOption(dgettext('dialogs','Delete directory'),
    Format(dgettext('dialogs','Delete "%s"'),[s]),mtConfirmation,[fsBold],
  ...
  if n>=0 then begin
    ...
    if n=0 then begin
      ...
      err:=ec>0;
      if not err then InfoDialog(Format(dgettext('dialogs','%u directories with %u files deleted!'),[dc,fc]));
      end
    else begin
      err:=ShellDeleteAll (Application.Handle,SetDirName(s),
                   Format(dgettext('dialogs','Deleting: %s'),[s]),true)<>NO_ERROR;
      if not err then InfoDialog(Format(dgettext('dialogs','%s moved to Recycle Bin!'),[s]));
      end;
    if err then ErrorDialog(Format(dgettext('dialogs','Error deleting directory:'+sLineBreak+'%s!'),[s]));
...
begin
  ...
  ShellListView.ObjectTypes:=ShellTreeView.ObjectTypes;
...
begin
  ...
  Result:=ShellDirDialog.Execute(ATitle,Hidden,FileView,HomeDir,Dir);
  end;
